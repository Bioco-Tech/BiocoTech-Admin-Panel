...
function Singlecompany({ data }) {
    ...
    const [editVid, setEditVid] = useState(data.id)
    const [editVpin, setEditVpin] = useState(data.pin)
    const [editVabout, setEditVabout] = useState(data.about)
    ...
    const focusId = () => {
        if (editId == false)
            inputElement_1.current.focus()


    }
    const focusPin = () => {
        if (editPin == false)
            inputElement_2.current.focus()


    }
    const focusAbout = () => {
        if (editAbout == false)
            inputElement_3.current.focus()


    }
    const focusAttach = () => {
        if (editAttach == false)
            inputElement_4.current.focus()
    ...
    const sendPostRequest = () => {
        ...
        if (edit == true) {
            console.log("hello1")
            axios.put(`http://localhost:5000/api/companies/${data._id}`, { name: editValue});
            {data.name}
        }
    }
    const sendPostId = () => {
        ...
        if (editId == true) {
            console.log("hello2")
            axios.put(`http://localhost:5000/api/companies/${data._id}`, { id: editVid })
        }
    }
    const sendPostPin = () => {
        // console.log(saveOrEdit)
        if (editPin == true) {
            console.log("hello3")
            axios.put(`http://localhost:5000/api/companies/${data._id}`, {  pin: editVpin })
        }
    }
    const sendPostAbout = () => {
        // console.log(saveOrEdit)
        if (editAbout == true) {
            console.log("hello4")
            axios.put(`http://localhost:5000/api/companies/${data._id}`, {  about: editVabout })
        }
    }
    const sendPostAttach = () => {
        ...
        if (editAttach == true) {
            console.log("hello4")
            axios.put(`http://localhost:5000/api/companies/${data._id}`, {  attach: editVattach })
        }
    }
    ...
    {/*} const fetchCompany = async (context) => {
        ...
        await axios.get(`http://localhost:5000/api/companies/${id}`).then((res)=>{
    ...
    return (
        <>
                    ...
                    <form className="space-y-6" onSubmit={(e) => handleSubmit(e)}>
                        <div>
                            <div className="py-4 sm:grid px-16 sm:grid-cols-3 border-b md:ml-32 md:mr-32 md:mt-32 lg:mr-64 lg:mt-32 border-gray-200 sm:gap-4    sm:py-5">
                                <dt className="text-sm font-medium font-abc text-gray-500">Unit</dt>
                                <dd className="mt-1 flex text-sm text-gray-900 sm:col-span-2 sm:mt-0">
                                    ...
                                    <span className="  flex flex-shrink-0 space-x-4">
                                        ...
                                        <span className="text-gray-300 font-abc  mt-2" aria-hidden="true">
                                            |
                                        </span>
                                        <button
                                            type="button"
                                ...
                                <dt className="text-sm font-medium mt-1 font-abc  text-gray-500">Unit ID</dt>
                                <dd className="mt-1 flex text-sm text-gray-900 sm:col-span-2 sm:mt-0">
                                    <input
                                ...
                                <dt className="text-sm font-medium text-gray-500 font-abc ">Unit Details</dt>
                                <dd className="mt-1 flex text-sm text-gray-900 sm:col-span-2 sm:mt-0">
            ...
            {!openModal && <div>
                ...
                <button className="text-lg ml-5 mt-5 font-medium leading-6 text-gray-900" ><span className=' text-md'> <Link className='flex font-abc  justify-center flex-row items-center' href='/admin/companies'><IoIosArrowBack />Back</Link></span></button>
                    ...
                    <div className='ml-5 mr-5 mt-5 mb-5 w-1/2'>
                        ...
                        <div className="mt-5  border-gray-200">
                            <dl className="divide-y divide-lime-200">
                                ...
                                <div className="py-4 sm:grid sm:grid-cols-3 sm:gap-4 sm:py-5">
                                    <dt className="text-sm font-medium font-abc  text-gray-500">Status</dt>
                                    <dd className="mt-1 flex text-sm text-gray-900 sm:col-span-2 sm:mt-0">
                                ...
                                <div className="py-4 sm:grid sm:grid-cols-3 sm:gap-4 sm:py-5">
                                    <dt className="text-sm font-medium font-abc  text-gray-500">Web/Link(options)</dt>
                                    <dd className="mt-1 flex text-sm text-gray-900 sm:col-span-2 sm:mt-0">
                                        <span className="flex-grow font-abc ">{data.website}</span>
                                        ...
                                        <ul role="list" className="divide-y divide-gray-200 rounded-md border border-gray-200">
                                            <li className="flex items-center justify-between py-3 pl-3 pr-4 text-sm">
                                                <div className="flex w-0 flex-1 items-center">
                                                    ...
                                                    <span className="ml-2 font-abc w-0 flex-1 truncate">resume_back_end_developer.pdf</span>
                                                </div>
                                            ...
                                            <li className="flex items-center justify-between py-3 pl-3 pr-4 text-sm">
                                                <div className="flex w-0 flex-1 items-center">
                                                    ...
                                                    <span className="ml-2 w-0 flex-1 font-abc  truncate"><input ref={inputElement_4} value={editAttach ? editVattach:'coverletter_back_end_developer.pdf' } onChange={(e) => { setEditVattach(e.target.value) }} /></span>
                                                </div>
                                                <div className="ml-4 flex flex-shrink-0 space-x-4">
                                                    <button
                                                     onClick={() => { setEditAttach(!editAttach); focusAttach(); sendPostAttach() }}
                                                        type="button"
                                ...
                                <div className="py-4 sm:grid sm:grid-cols-3 border-b border-gray-200 sm:gap-4 sm:py-5"> <dt className="text-sm font-medium text-gray-500 font-abc ">Unit</dt>



                                    <dd className="mt-1 flex text-sm text-gray-900 sm:col-span-2 sm:mt-0">
                                        <span className="flex-grow font-abc "> <Menu as="div" className="relative inline-block text-left">
                                            <div>
                                    ...
                                    </dd>
                                    {
                                        items.map((unit) => (
                                            <>
                                                <dt className="text-sm font-medium text-gray-500 font-abc ">Unit ID</dt>
                                                <dd className="mt-1 flex text-sm text-gray-900 sm:col-span-2 sm:mt-0 font-abc ">{unit.id}</dd>
                                                <dt className="text-sm font-medium text-gray-500 font-abc ">Unit Details</dt>
                                                <dd className="mt-1 flex text-sm text-gray-900 sm:col-span-2 sm:mt-0 font-abc ">{unit.unit_details}</dd>
                                            </>
                                        ))
                                    }
                            ...
                            <div className="flex flex-1 border border-transparent  justify-between mt-0 sm:px-2 px-4 md:px-0">
                                <div className="flex flex-1">
                                    <form className="flex w-full md:ml-0" action="#" method="GET">
                                        <label htmlFor="search-field" className="sr-only text-cyan-400 font-abc ">
                                            Search Staff Member
                                        </label>
                        ...
                        <div className="px-4 sm:px-6 py-4 sm:py-6 lg:px-8 lg:py-8 rounded-lg bg-gray-100">
                            <div className="sm:flex sm:items-center ">
                                <div className="sm:flex-auto">
                                    <h1 className="text-xl font-semibold text-gray-900 font-abc ">Pizza Hut</h1>
                                            ...
                                            <table className="min-w-full divide-y divide-gray-300">
                                                <thead className="bg-lime-500 ">
                                                    <tr>
                                                        ...
                                                        <th scope="col" className="px-3 py-3.5 text-left text-sm font-semibold font-abc text-white">
                                                ...
                                                <tbody className="divide-y divide-gray-200 bg-lime-50">
                                                    {staff.map((person) => (
                                                        <tr key={person.name}>
                                                            ...
                                                            <td className="whitespace-nowrap px-3 py-4 text-sm font-abc  text-gray-500">{person.date}</td>
                                                            <td className="whitespace-nowrap px-3 py-4 text-sm font-abc  text-gray-500">{person.ibs}</td>
                                                            <td className="whitespace-nowrap px-3 py-4 text-sm font-abc  text-gray-500">{person.mtco}</td>
...
export async function getServerSideProps(context) {
    ...
    const res = await fetch(`http://localhost:5000/api/companies/${_id}`)
...
{/*export async function getServerSideProps(context) {
    const { _id } = context.query;
    console.log(context.query._id)
    const res = await fetch(`http://localhost:5000/api/companies/units/${_id}`)
    const data = await res.json();
